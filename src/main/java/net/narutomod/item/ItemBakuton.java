...
public class ItemBakuton extends ElementsNarutomodMod.ModElement {
	...
	public static final ItemJutsu.JutsuEnum CLAY = new ItemJutsu.JutsuEnum(1, "c_1", 'S', 200, 150d, new ExplosiveClay.Jutsu());
	public static final ItemJutsu.JutsuEnum CLONE = new ItemJutsu.JutsuEnum(2, "explosive_clone", 'S', 200, 150d,
			new EntityExplosiveClone.EC.Jutsu());
	...
	public void initElements() {
		...
		// .id(new ResourceLocation("narutomod", "entitybulletbakuton"),
		// ENTITYID).name("entitybulletbakuton").tracker(64, 1, true).build());
	}
	...
	public static class RangedItem extends ItemJutsu.Base {
		...
		public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer entity, EnumHand hand) {
			if (entity.isCreative()
					|| (ProcedureUtils.hasItemInInventory(entity, ItemDoton.block) && ProcedureUtils.hasItemInInventory(entity, ItemRaiton.block))) {
		...
		public void onUpdate(ItemStack itemstack, World world, Entity entity, int par4, boolean par5) {
			...
			if (entity.ticksExisted % 10 == 2 && entity instanceof EntityLivingBase && JIRAIKEN.jutsu.isActivated(itemstack)) {
				((EntityLivingBase) entity).addPotionEffect(new PotionEffect(PotionChakraEnhancedStrength.potion, 12,
						(int) (((Jiraiken) JIRAIKEN.jutsu).getPower(itemstack) * 19), false, false));
			}
		}
		...
		public void addInformation(ItemStack itemstack, World world, List<String> list, ITooltipFlag flag) {
			...
			list.add(TextFormatting.GREEN + net.minecraft.util.text.translation.I18n.translateToLocal("tooltip.bakuton.musthave")
	...
	public static class Jiraiken implements ItemJutsu.IJutsuCallback {
		...
		public boolean createJutsu(ItemStack stack, EntityLivingBase entity, float powerIn) {
			...
			// !entity.isPotionActive(PotionChakraEnhancedStrength.potion);
			return flag;
		}
	...
	public abstract static class ExplosiveClay extends EntityCreature implements ItemJutsu.IJutsu {
		...
		public boolean attackEntityAsMob(Entity entityIn) {
			...
			if (!this.world.isRemote) {
				this.world.createExplosion(owner, this.posX, this.posY, this.posZ, this.explosionSize,
						net.minecraftforge.event.ForgeEventFactory.getMobGriefingEvent(this.world, owner));
				this.setDead();
				return true;
			}
			return false;
		...
		private void poof() {
			...
			for (int i = 0; i < 200; i++) {
				...
				particles.spawnParticles(Particles.Types.SMOKE, this.posX + (this.rand.nextFloat() - 0.5f) * this.width,
						this.posY + this.rand.nextFloat() * this.height, this.posZ + (this.rand.nextFloat() - 0.5f) * this.width, 1, 0d, 0d, 0d, 0d,
						0d, 0d, -1, (int) (scale * 10f), 0, 0, -1, -6 - this.rand.nextInt(8));
			}
			particles.send();
		}
		...
		public static class Jutsu implements ItemJutsu.IJutsuCallback {
			...
			public boolean createJutsu(ItemStack stack, EntityLivingBase entity, float powerIn) {
				...
				} else if (powerIn < 4f && entity.getEntityData().getDouble(NarutomodModVariables.BATTLEXP) >= 20000) {
					ec = new EntityC3.EC(entity);
				} else if (powerIn <= this.getMaxPower() && entity.getEntityData().getDouble(NarutomodModVariables.BATTLEXP) >= 32000) {
		...
		class AIChargeAttack extends EntityAIBase {
			...
			public boolean shouldExecute() {
				...
				if (target != null && !ExplosiveClay.this.getMoveHelper().isUpdating() && ExplosiveClay.this.rand.nextInt(5) == 0) {
