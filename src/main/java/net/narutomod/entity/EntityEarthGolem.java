...
public class EntityEarthGolem extends ElementsNarutomodMod.ModElement {
	...
	public static class EC extends EntitySummonAnimal.Base implements ItemJutsu.IJutsu {
		...
		public EC(EntityLivingBase summonerIn, float size) {
			...
			this.isImmuneToFire = true;
			this.setScale(size);
			this.setLocationAndAngles(summonerIn.posX + summonerIn.getLookVec().x, summonerIn.posY, summonerIn.posZ + summonerIn.getLookVec().z,
					summonerIn.rotationYaw, 0f);
		}
		...
		protected void onDeathUpdate() {
			if (!this.world.isRemote) {
				...
				Particles.spawnParticle(this.world, Particles.Types.BLOCK_DUST, this.posX, this.posY + this.height * 0.5, this.posZ,
						(int) (this.getScale() * 1000), 0.5d * this.width, 0.3d * this.height, 0.5d * this.width, 0d, 0d, 0d,
						Block.getIdFromBlock(Blocks.DIRT), 40);
			}
			this.setDead();
		}
				...
				if (age <= this.growTime) {
					for (int i = 0; i < (int) (this.getScale() * 20); i++) {
						this.world.spawnParticle(EnumParticleTypes.BLOCK_DUST, this.posX, this.posY, this.posZ,
								(this.rand.nextDouble() - 0.5d) * this.width * 0.25d, 0.15d, (this.rand.nextDouble() - 0.5d) * this.width * 0.25d,
			...
			public boolean createJutsu(ItemStack stack, EntityLivingBase entity, float power) {
				if (power >= 1.0f) {
					if (entity instanceof EntityPlayer) {
						ItemJutsu.setCurrentJutsuCooldown(stack, entity, (long) (power * 1200));
					}
	...
	public static class Renderer extends EntityRendererRegister {
		...
		public class ModelRockGolem extends ModelBase {
			...
			public void setLivingAnimations(EntityLivingBase entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTickTime) {
				...
				this.ironGolemHead.setRotationPoint(this.headStart.x + (this.headEnd.x - this.headStart.x) * f,
						this.headStart.y + (this.headEnd.y - this.headStart.y) * f, this.headStart.z + (this.headEnd.z - this.headStart.z) * f);
				this.ironGolemBody.setRotationPoint(this.bodyStart.x + (this.bodyEnd.x - this.bodyStart.x) * f,
						this.bodyStart.y + (this.bodyEnd.y - this.bodyStart.y) * f, this.bodyStart.z + (this.bodyEnd.z - this.bodyStart.z) * f);
				this.ironGolemRightArm.setRotationPoint(this.rightArmStart.x + (this.rightArmEnd.x - this.rightArmStart.x) * f,
						this.rightArmStart.y + (this.rightArmEnd.y - this.rightArmStart.y) * f,
						this.rightArmStart.z + (this.rightArmEnd.z - this.rightArmStart.z) * f);
				this.ironGolemLeftArm.setRotationPoint(this.leftArmStart.x + (this.leftArmEnd.x - this.leftArmStart.x) * f,
						this.leftArmStart.y + (this.leftArmEnd.y - this.leftArmStart.y) * f,
						this.leftArmStart.z + (this.leftArmEnd.z - this.leftArmStart.z) * f);
				this.ironGolemRightLeg.setRotationPoint(this.rightLegStart.x + (this.rightLegEnd.x - this.rightLegStart.x) * f,
						this.rightLegStart.y + (this.rightLegEnd.y - this.rightLegStart.y) * f,
						this.rightLegStart.z + (this.rightLegEnd.z - this.rightLegStart.z) * f);
				this.ironGolemLeftLeg.setRotationPoint(this.leftLegStart.x + (this.leftLegEnd.x - this.leftLegStart.x) * f,
						this.leftLegStart.y + (this.leftLegEnd.y - this.leftLegStart.y) * f,
						this.leftLegStart.z + (this.leftLegEnd.z - this.leftLegStart.z) * f);
