					...
					while (iter.hasNext()) {
						...
						if (ItemJutsu.canTarget(entity)) {
							...
							if (age > this.growTime - 5) {
								entity.attackEntityFrom(DamageSource.IN_WALL, age > this.growTime ? 0.3f : (f * f * 0.05f));
							}
				...
				if (power >= 2f) {
					...
					if (rt != null && rt.typeOfHit == RayTraceResult.Type.BLOCK) {
						if (entity instanceof EntityPlayer) {
							ItemJutsu.setCurrentJutsuCooldown(stack, (EntityPlayer) entity, (long) (power * 1200));
						}
						if (power >= 3f) {
