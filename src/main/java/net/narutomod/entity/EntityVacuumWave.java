...
public class EntityVacuumWave extends ElementsNarutomodMod.ModElement {
	...
	public void initElements() {
		elements.entities.add(() -> EntityEntryBuilder.create().entity(EC.class).id(new ResourceLocation("narutomod", "vacuum_wave"), ENTITYID)
				.name("vacuum_wave").tracker(64, 3, true).build());
	}
		...
		public void onUpdate() {
			...
			if (!this.world.isRemote) {
				if (this.ticksExisted == 1) {
					this.playSound(SoundEvent.REGISTRY.getObject(new ResourceLocation("narutomod:windblast")), 0.8f,
							0.4f + this.rand.nextFloat() * 0.9f);
				}
				...
				if (this.ticksExisted > this.maxAge) {
					...
					for (EntityLivingBase entity : this.world.getEntitiesWithinAABB(EntityLivingBase.class,
							this.getEntityBoundingBox().expand(0d, -this.height, 0d))) {
						if (entity != this.shootingEntity && !entity.equals(this) && entity.getDistance(this) <= 0.5d * this.width) {
							ProcedureUtils.Vec2f vec = ProcedureUtils
									.getYawPitchFromVec(entity.getPositionVector().subtract(this.getPositionVector()));
							vec = vec.subtract(this.rotationYaw, this.rotationPitch);
				            if (Math.abs(vec.x) <= 90f && Math.abs(vec.y) <= 90f) {
							 	entity.hurtResistantTime = 10;
								entity.attackEntityFrom(ItemJutsu.causeJutsuDamage(this, this.shootingEntity), 5.0f + scale);
				            }
						}
					}
				}
			...
			public boolean createJutsu(ItemStack stack, EntityLivingBase entity, float power) {
				if (power >= 1.0F) {
					...
					if (entity instanceof EntityPlayer) {
						ItemJutsu.setCurrentJutsuCooldown(stack, (EntityPlayer) entity, (long) (power * 240));
					}
					return true;
				}
				return false;
	...
	public static class Renderer extends EntityRendererRegister {
		...
		public class RenderCustom extends EntitySweep.Renderer.RenderCustom {
			...
			protected void renderParticles(EntitySweep.Base entity, Vec3d entityVec, Vec3d relVec, int color, float scale) {
				...
				Particles.spawnParticle(entity.world, Particles.Types.SMOKE, vec1.x, vec1.y, vec1.z, 1, 0d, 0d, 0d, vec2.x, vec2.y, vec2.z,
						(0x10 << 24) | (color & 0x00FFFFFF), (int) (scale * 5), (int) (8.0d / (entity.rand().nextDouble() * 0.8d + 0.2d)));
			}
		}
	}
}
